...
BuildImageController.create = (req, res) => {
  ...
  UtilService.wrapCb(BuildImage.create(params), (err, buildImage) => {
    ...
    return SourceManagement.findById(buildImage.sourceControlId).then((repo)=>{
      ...
      console.log('This is repository github.js link',repo.url);
           ...
           console.log('This is inside shell script function');
             ...
             setTimeout(function () {
               if (shell.stdout === 'Successfully cloned') {
                 buildImage.setDataValue('status', 'Success Clonned');
                 console.log('Status inside function success clonned');
                 return buildImage.save();


               }
               else {
                 buildImage.setDataValue('status', 'Failed to cloned');
                 console.log('Status inside function failed clonned');
                 return buildImage.save();
