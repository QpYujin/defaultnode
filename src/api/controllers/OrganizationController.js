...
OrganizationController.getconfig = (req, res) => {
let params = req.body;
console.log('This is at the getconfig cluster controller',params);
UtilService.wrapCb(Cluster.create(params), (err, cluster) => {
    if (err) {
      server.log.error('Error create cluster', err);
      res.status(500).json(err);
...
console.log("at the server side constroller for get config cluster");
console.log("cluster access :",params.access);
console.log("cluster secret :",params.secret);

shell.exec('/usr/src/app/api/controllers/scripts/getconfig.sh'+' '+params.access+' '+params.secret,
  function (error, stdout, stderr) {
       console.log('This is inside shell script function');
              if (error !== null) {
                            console.log('exec error: ' + error);
